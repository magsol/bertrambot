...
void ExampleAIModule::onReceiveText(BWAPI::Player* player, std::string text)
{
  ...
  if (text.compare("n00b") == 0 || text.compare("nub") == 0) // ATTACK!!!!
  {
	for(std::set<Unit*>::const_iterator i = Broodwar->self()->getUnits().begin(); i != Broodwar->self()->getUnits().end(); i++)
	{
		if ((*i)->getType().canAttack())
		{
			Position enemyPos = Position(player->getStartLocation());
			(*i)->attackMove(enemyPos);
		}
	}
	Broodwar->sendText("%s, you're dead man.", player->getName().c_str());
  }
  // end heh heh
}
